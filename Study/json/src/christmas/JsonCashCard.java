 ...
 *   author:zhanglianyu
 *   date:2016年12月23日
 *   以下代码测试搜狗阅读圣诞活动兑换卡片接口，测试实物奖和搜豆的获取概率和获取限制是否正确
 *   主要以预先写入参数，对比抽奖接口返回json中参数是否和预先设定好的参数相等
 */
public class JsonCashCard {
	public static void main(String[] args) throws InterruptedException {
		...
		int yamaxun50Num = 2;
		int huafei50Num = 5;
		int jinwangzaiCountNum = 10;
		// 卡片
		int gouNum = 2000;
		int baoNum = 200;
		// 四字礼包
		int wangzaiyusanNum = 20;
		int aoyunwangzaiNum = 9;
		int yundongerjiNum = 4;
		// 7字礼包
		int TxueNum = 100;
		int UpanNum = 7;
		int yamaxun100Num = 7;
		...
		while (true) { // 开启循环，一下变量需要初始化
			...
			StringText ppid;
			if(aa==1){
			 	ppid = "zhanglianyu1@sogou.com";
				aa=2;
			}else{
			 	ppid = "17710893436@sohu.com";
			 	aa=1;
			 	}
			 * 
			 */
			...
			// 兑换卡片
			String urlDuiHuan = "http://ot.yuedu.sogou.com/abs/activity/christmas/ajax/exchange?cuuid=869042020141273&imsi=&"
					+ "ppid=" + ppid + "&versioncode=3701&s=&token=e4da66f2057dcd220d8e17889350e067&eid=9999&gender=0";
			//StringText json = loadJSON(url);// 访问url并获取json
			String jsonDuiHuan = loadJSON(urlDuiHuan);
			System.out.println(jsonDuiHuan);
			String arrjson[];
			String reward;
			byte status = 0; // 判断是否是回到起点
			// 获取reward
			if (jsonDuiHuan.contains("\"reward\":") && jsonDuiHuan.contains("gridId")) { // 分割json
				arrjson = jsonDuiHuan.split("\"reward\":");
				...
				if (id != rewardID) { // 判断id
					JOptionPane.showMessageDialog(null,
							"rewardGridID与rewardID不相符;rewardGridID:" + rewardGridID + ";rewardID:" + rewardID, "提示",
							JOptionPane.WARNING_MESSAGE);
				}
			} else if (jsonDuiHuan.contains("{\"status\":\"succ\",\"startPos\":") && jsonDuiHuan.contains("\"endPos\":0")) {
				huidaoqidian = huidaoqidian + 1; // 回到起点
				status = 1;
			} else if (jsonDuiHuan.contains("用户无效")) {// 判断用户信息是否有效
				...
				if (err == index && (jsonDuiHuan.contains("用户无效") == false) && usercount > 3) {// 连续发生两次错误弹出提示框
			...
			if (status == 0) { // 判断是否是回到起点，如果是，不执行奖品对比操作
				JSONObject a = JSONObject.fromObject(jsonDuiHuan);
				...
				if (rewardGridID == gridID) { // 对比gridId和奖品gridId是否相等
					...
					if (rewardGridID == 1) { // 金汪仔
						...
						case 7:
							...
							break;
						default:
							JOptionPane.showMessageDialog(null, "gridID:" + gridID + ";rewardGridID:"
									+ rewardGridID + ";rewardID:" + rewardID, "提示", JOptionPane.WARNING_MESSAGE);
							break;
						}
						...
						case 33:
							...
							break;
						default:
							JOptionPane.showMessageDialog(null, "没有找到rewardGridID" + rewardGridID, "提示",
									JOptionPane.WARNING_MESSAGE);
							break;
						}
						...
						case 34:
							...
							break;
						default:
							JOptionPane.showMessageDialog(null, "rewardID:" + "gridID:" + gridID + ";rewardGridID:"
									+ rewardGridID + ";rewardID:" + rewardID, "提示", JOptionPane.WARNING_MESSAGE);
							break;
						}
					} else {
					...
					if (i != 0) { // 验证rewardID对应的type类型和gl是否正确
						JOptionPane.showMessageDialog(null,
								"type或gl不正确:" + "gridID:" + gridID + ";rewardGridID:" + rewardGridID + ";rewardID:"
										+ rewardID + ";type" + type + ";gl:" + gl + ";i:" + i,
					...
					JOptionPane.showMessageDialog(null,
							"gridID不相等:" + "gridID:" + gridID + ";rewardGridID:" + rewardGridID, "提示",
							JOptionPane.WARNING_MESSAGE);
				}
			}
			// 金汪仔的实物奖励判断
			...
			if (sogouwano > sogouwanoNum) {
				JOptionPane.showMessageDialog(null, "搜狗玩偶赠送超标了,sogouwanoRewardId:" + sogouwano + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			System.out.println("搜狗抱枕获取次数:" + dingzhibaozhen);
			if (dingzhibaozhen > dingzhibaozhenNum) {
				JOptionPane.showMessageDialog(null,
						"搜狗抱枕赠送超标了,dingzhibaozhenRewardId:" + dingzhibaozhen + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			System.out.println("搜狗笔记本获取次数:" + sogoubijiben);
			if (sogoubijiben > sogoubijibenNum) {
				JOptionPane.showMessageDialog(null, "搜狗笔记本赠送超标了,sogoubijibenRewardId:" + sogoubijiben + ";date:" + date,
						"提示", JOptionPane.WARNING_MESSAGE);
			}
			System.out.println("搜狗50亚马逊获取次数:" + yamaxun);
			if (yamaxun > yamaxun50Num) {
				JOptionPane.showMessageDialog(null, "搜狗50亚马逊赠送超标了,yamaxunRewardId:" + yamaxun + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			System.out.println("搜狗50话费获取次数:" + huafei);
			if (huafei > huafei50Num) {
				JOptionPane.showMessageDialog(null, "搜狗50话费赠送超标了,huafeiRewardId:" + huafei + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			int number = (sogouwano + dingzhibaozhen + sogoubijiben + yamaxun + huafei);
			if (number > jinwangzaiCountNum) {
				JOptionPane.showMessageDialog(null, "搜狗金汪仔总数赠送超标了,count:"
						+ (sogouwano + dingzhibaozhen + sogoubijiben + yamaxun + huafei) + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			...
			if (xiaofanjiangli[user] > 1) {
				JOptionPane.showMessageDialog(null,
						"银汪仔消费获得次数大于1，" + "xiaofanjiangli:" + xiaofanjiangli[user] + "user:" + ppid + ";date:" + date,
						"提示", JOptionPane.WARNING_MESSAGE);
			}
			System.out.println();
			// 搜豆
			...
			System.out.println();
			if (gou > gouNum) {
				JOptionPane.showMessageDialog(null, "狗卡片赠多了，gou" + gou + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			if (bao > baoNum) {
				JOptionPane.showMessageDialog(null, "包卡片赠多了，bao" + bao + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			...
			if (wangzaiyusan > wangzaiyusanNum) {
				JOptionPane.showMessageDialog(null, "雨伞赠多了，wangzaiyusan" + wangzaiyusan + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
			if (aoyunwangzai > aoyunwangzaiNum) {
				JOptionPane.showMessageDialog(null, "奥运赠多了，aoyunwangzai" + aoyunwangzai + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);

			}
			if (yundongerji > yundongerjiNum) {
				JOptionPane.showMessageDialog(null, "运动耳机赠多了，yundongerji" + yundongerji + ";date:" + date, "提示",
						JOptionPane.WARNING_MESSAGE);
			}
