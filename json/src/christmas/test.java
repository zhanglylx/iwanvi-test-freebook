...
public class test {
	public static void main(String[] args) throws InterruptedException {
		int index = 0; // 循环次数
		int n = 9;// 指定抽奖骰子数 //需要到抽奖URL请求打开pos
		while (true) {
			Date dt = new Date();
			SimpleDateFormat matter1 = new SimpleDateFormat("yyyyMMdd");
			System.out.println(matter1.format(dt));
			// Thread.sleep(10000);
			if (matter1.format(dt).contains("1222")) { // 检查日期
				break;
			}
		}
		...
		// 金汪仔
		int sogouwanoNum = 12;
		int dingzhibaozhenNum = 12;
		int sogoubijibenNum = 12;
		int yamaxun50Num = 12;
		int huafei50Num = 15;
		int jinwangzaiCountNum = 18;
		// 卡片
		int gouNum = 0;
		int baoNum = 0;
		// 四字礼包
		...
		while (true) { // 开启循环，一下变量需要初始化
			...
			String ppid = "13426283025@sohu.com";
